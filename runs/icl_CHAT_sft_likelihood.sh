...
if [ -f "$file_path" ]; then
    ...
    while IFS= read -r line || [ -n "$line" ]; do
        ...
        CUDA_VISIBLE_DEVICES=6 python src/main.py --mode sft_likelihood --target_persona $line --exp_name llama-chat_sft_likelihood --model_dir /nobackup2/froilan/checkpoints/llama-2/Llama-2-7b-chat-hf/ --max_input_len 300 --output_dir /nobackup2/froilan/checkpoints/persona_sft_label_agnostic/ --likelihood_use_epoch 4
