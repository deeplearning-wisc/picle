...
def test_model(args, model, test_dataset, doa_test_dataset=None):
    ...
    avg_conf = eval_confidence(preds, probs)
    total_entropy, binary_entropy = eval_uncertainty(probs)
...
def eval_confidence(preds, probs):
    prob_list = []
    for output, prob in zip(preds, probs):
        if output == 1:
            prob_list.append(get_yes_no_total_prob(prob)[0])
        elif output == 0:
            prob_list.append(get_yes_no_total_prob(prob)[1])
        else :
            continue
    if len(prob_list) == 0 :
        return 0.0
...
def eval_uncertainty(probs):
    totents, binents = [], []
    for prob in probs:
        if prob is None :
            continue
        yes_p, no_p = get_yes_no_total_prob(prob)
        yes_p_norm, no_p_norm = yes_p/(yes_p+no_p), no_p/(yes_p+no_p)
