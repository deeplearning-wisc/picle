...
def load_opt(model_name_or_path, memory_for_model_activations_in_gb=2, peft_path=None):
...
class OPTWrapper(object):
    def __init__(self, model_dir, lora_adapter_path=None, memory_for_model_activations_in_gb=2):
        ...
        self.name = model_dir
        self.huggingface_model = load_opt(model_dir, memory_for_model_activations_in_gb, lora_adapter_path)
        ...
        # self.tokenizer.eos_token = self.tokenizer.convert_ids_to_tokens(50260)
    ...
    def generate(self, args, query, return_logits=True, verbose=False):
        ...
        if return_logits:
            map_dict = {
                'yes':[10932,4420], 'yes.':[10932,4420], 'Yes':[9904,3216], 'Yes.':[9904,3216],
                'YES':[41010,32463], 'YES.':[41010,32463], 'no':[2362,117], 'no.':[2362,117],
                'No':[3084,440], 'No.':[3084,440], 'NO':[13449,8228], 'NO.':[13449,8228]
            }
