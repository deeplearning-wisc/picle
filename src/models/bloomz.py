...
import torch
import tqdm
from transformers import AutoConfig, AutoModelForCausalLM, BloomTokenizerFast, DataCollatorWithPadding, pipeline
...
def load_bloomz(model_name_or_path, memory_for_model_activations_in_gb=2, peft_path=None):
...
class BloomzWrapper(object):
    def __init__(self, model_dir, lora_adapter_path=None, memory_for_model_activations_in_gb=2):
        ...
        self.huggingface_model = load_bloomz(model_dir, memory_for_model_activations_in_gb, lora_adapter_path)
        self.tokenizer = BloomTokenizerFast.from_pretrained(model_dir, legacy=False)
    ...
    def generate(self, args, query, return_logits=True, verbose=False):
        ...
        # <0x0A><0x0A>
        response = response.replace('Ġ',' ').replace('</s>', '').strip()
        ...
        if return_logits:
            map_dict = {
                'yes':[18260,34474], 'yes.':[18260,34474], 'Yes':[31559,31830], 'Yes.':[31559,31830],
                'no':[654,1936], 'no.':[654,1936], 'No':[3928,4309], 'No.':[3928,4309]
            }
