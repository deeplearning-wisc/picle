...
if [ -f "$file_path" ]; then
    ...
    while IFS= read -r line || [ -n "$line" ]; do
        ...
        CUDA_VISIBLE_DEVICES=7 python src/main.py --mode sft_likelihood --likelihood_func diff --target_persona $line --exp_name bloomz_picle --model_dir /nobackup2/froilan/checkpoints/bloomz/bloom-7b1/ --model bloomz --output_dir /nobackup2/froilan/checkpoints/bloomz_sft/persona_sft_label_agnostic/ --likelihood_use_epoch 2
...
# CUDA_VISIBLE_DEVICES=1 python src/main.py --mode sft_likelihood --likelihood_func diff --target_persona narcissism --exp_name mistral_picle --model_dir /nobackup2/froilan/checkpoints/mistral/Mistral-7B-Instruct-v0.1/ --model mistral --max_input_len 400 --output_dir /nobackup2/froilan/checkpoints/mistral_sft/persona_sft_label_agnostic/ --likelihood_use_epoch 10

    # CUDA_VISIBLE_DEVICES=2 python src/main.py --mode sft_likelihood --likelihood_func diff --target_persona psychopathy --exp_name mistral_picle --model_dir /nobackup2/froilan/checkpoints/mistral/Mistral-7B-Instruct-v0.2/ --model mistral --output_dir /nobackup2/froilan/checkpoints/mistral_sft/persona_sft_label_agnostic/ --likelihood_use_epoch 10